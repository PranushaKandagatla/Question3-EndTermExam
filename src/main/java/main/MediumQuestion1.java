...
class SeniorCitizenCustomer extends Customer {
	...
	Double getBillAmount(Double amount) {
		Double bill = amount - ((amount/100)*10); //do ofr other with 10% discount
...
class PrivilegeCustomer extends Customer {
	PrivilegeCustomer(String name, String address, Integer age, String mobile) {
		...
		Double getBillAmount(Double amount) {
			Double bill = amount - ((amount/100)*30); //30% discount for privileged coustmener
	...
	public static void main(String[] args) {
		...
		switch (select) {
			...
			case 2:
				...
				System.out.println(cust1.toString()+"\nYour bill amount is Rs. "+purchasedAmt+". Congrats! As you are a Senior Citizen customer, you are eligible for a discount!\nYou have to pay Rs."+cust1.getBillAmount(purchasedAmt));
				break;
